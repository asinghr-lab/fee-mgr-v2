...
public class AcademicYearService {
	...
	public AcademicYear getActive() {
		return repository.findFirstByActiveTrueOrderByStartDateDesc().orElse(null);
	}

	@Transactional(readOnly = true)
	public AcademicYear requireActive() {
		return repository.findFirstByActiveTrueOrderByStartDateDesc()
				.orElseThrow(() -> new IllegalStateException("No active academic year is configured."));
	}

	@PreAuthorize("hasRole('ADMIN')")
	@Transactional
	public AcademicYear create(String name, LocalDate start, LocalDate end) {
		String n = name == null ? "" : name.trim();
		if (n.isBlank())
			throw new IllegalArgumentException("Academic year name is required.");
		if (!end.isAfter(start))
			throw new IllegalArgumentException("End date must be after start date");
		repository.findByNameIgnoreCase(n).ifPresent(x -> {
			throw new IllegalArgumentException("An academic year with this name already exists.");
		});
		repository.findByActiveTrueOrderByStartDateDesc().forEach(AcademicYear::deactivate);
		AcademicYear year = new AcademicYear(n, start, end);
		year.activate();
	...
	public void activate(Long id) {
		AcademicYear target = repository.findById(id)
				.orElseThrow(() -> new IllegalArgumentException("Academic year not found."));
		repository.findByActiveTrueOrderByStartDateDesc().forEach(y -> {
	...
	public void deactivate(Long id) {
		...
		if (target.isActive())
			throw new IllegalStateException(
					"The active academic year cannot be deactivated. Activate another academic year first.");
		target.deactivate();
	}
}
