...
public class IdentityWebController {
    ...
    public IdentityWebController(IdentityUserService userService, IdentityAdminService adminService) {
        this.userService = userService; this.adminService = adminService;
    }
    ...
    public String profile(Model model, Authentication authentication) {
        ...
        model.addAttribute("profileRequest", new ProfileRequest(profile.getDisplayName() == null ? "" : profile.getDisplayName()));
        model.addAttribute("profileUser", user);
        model.addAttribute("profileRoles", user.getUserRoles().stream().map(ur -> ur.getRole().name()).sorted().toList());
    ...
    public String resetPassword(@Valid @ModelAttribute("request") PasswordResetRequest request,
        ...
        if (bindingResult.hasErrors()) return "identity/reset-password";
        try {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public String createUser(@Valid @ModelAttribute("request") UserRequest request,
                             BindingResult bindingResult, Model model, RedirectAttributes redirect) {
        if (bindingResult.hasErrors()) { model.addAttribute("roles", Role.values()); return "identity/user-form"; }
        try {
            if (!request.password().equals(request.confirmPassword())) throw new IllegalArgumentException("Password and confirmation do not match.");
    ...
    public String changeStatus(@PathVariable Long id, @RequestParam boolean enabled,
        ...
        try { adminService.setEnabled(id, enabled, authentication.getName()); redirect.addFlashAttribute("message", enabled ? "User activated." : "User deactivated."); }
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public String adminResetPassword(@PathVariable Long id, @Valid @ModelAttribute("passwordRequest") AdminPasswordRequest request,
                                     BindingResult bindingResult, RedirectAttributes redirect) {
        if (bindingResult.hasErrors()) { redirect.addFlashAttribute("error", "Password must be at least 8 characters and both entries must be provided."); return "redirect:/admin/users/" + id; }
        if (!request.newPassword().equals(request.confirmPassword())) { redirect.addFlashAttribute("error", "New password and confirmation do not match."); return "redirect:/admin/users/" + id; }
