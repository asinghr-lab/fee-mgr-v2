...
public class IdentityRestController {
	...
	public IdentityRestController(UserRepository users, UserProfileRepository profiles, IdentityUserService userService,
			IdentityAdminService adminService) {
		this.users = users;
	...
	public org.springframework.data.domain.Page<?> users(@RequestParam(defaultValue = "0") int page,
		...
		return adminService.findUsers(PageRequest.of(Math.max(page, 0), safeSize)).map(u -> new UserSummary(u.getId(),
				u.getUsername(), u.isEnabled(), u.getUserRoles().stream().map(r -> r.getRole().name()).toList()));
	}
