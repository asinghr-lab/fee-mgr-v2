...
public class IdentityAdminService {
    ...
    public Page<User> findUsers(Pageable pageable) { return users.findAll(pageable); }

    @Transactional(readOnly = true)
    public User findUser(Long id) { return users.findDetailedById(id).orElseThrow(() -> new IllegalArgumentException("User not found.")); }

    @Transactional(readOnly = true)
    public UserProfile profile(Long id) { return profiles.findById(id).orElseThrow(() -> new IllegalArgumentException("User profile not found.")); }
    ...
    public void setEnabled(Long id, boolean enabled, String actingUsername) {
        User user = findUser(id);
        if (user.getUsername().equals(actingUsername) && !enabled) throw new IllegalStateException("You cannot deactivate your own account.");
        user.setEnabled(enabled); users.save(user);
    }
