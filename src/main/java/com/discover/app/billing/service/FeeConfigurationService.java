...
public class FeeConfigurationService {
	...
	public FeeConfigurationService(FeeComponentRepository components, FeeStructureRepository structures,
			GradeFeeStructureRepository assignments, GradeRepository grades) {
	...
	public FeeComponent createComponent(String name) {
		String n = name.trim();
		components.findByNameIgnoreCase(n).ifPresent(x -> {
			throw new IllegalArgumentException("A fee component with this name already exists.");
		});
	...
	public void deactivateComponent(Long id) {
		components.findById(id).orElseThrow(() -> new IllegalArgumentException("Fee component not found."))
				.deactivate();
	}

	@PreAuthorize("isAuthenticated()")
	public List<FeeComponent> allComponents() {
		return components.findAllByOrderByUpdatedAtDescIdDesc();
	}

	@PreAuthorize("isAuthenticated()")
	public List<FeeComponent> activeComponents() {
		return components.findByStatusOrderByNameAsc(FeeStatus.ACTIVE);
	}
	...
	public FeeStructure createStructure(String name, List<FeeStructureItemRequest> itemRequests) {
		String n = name.trim();
		structures.findByNameIgnoreCase(n).ifPresent(x -> {
			throw new IllegalArgumentException("A fee structure with this name already exists.");
		});
		if (itemRequests == null || itemRequests.isEmpty())
			throw new IllegalArgumentException("At least one fee structure item is required.");
		FeeStructure s = new FeeStructure(n);
		Set<Long> seen = new HashSet<>();
		for (var r : itemRequests) {
			if (!seen.add(r.feeComponentId()))
				throw new IllegalArgumentException("A fee component can occur only once in a structure.");
			FeeComponent c = components.findById(r.feeComponentId())
					.orElseThrow(() -> new IllegalArgumentException("Fee component not found."));
			if (c.getStatus() != FeeStatus.ACTIVE)
				throw new IllegalArgumentException("Only active fee components can be added to a new structure.");
	...
	public void deactivateStructure(Long id) {
		structures.findById(id).orElseThrow(() -> new IllegalArgumentException("Fee structure not found."))
				.deactivate();
	}

	@PreAuthorize("isAuthenticated()")
	public List<FeeStructure> allStructures() {
		return structures.findAllByOrderByUpdatedAtDescIdDesc();
	}

	@PreAuthorize("isAuthenticated()")
	public List<FeeStructure> activeStructures() {
		return structures.findByStatusOrderByNameAsc(FeeStatus.ACTIVE);
	}
	...
	@PreAuthorize("isAuthenticated()")
	public List<GradeFeeStructure> assignments() {
		return assignments.findAllForDisplay();
	}
	...
	public FeeComponentResponse componentResponse(FeeComponent c) {
		return new FeeComponentResponse(c.getId(), c.getName(), c.getStatus());
	}

	public FeeStructureResponse structureResponse(FeeStructure s) {
		return new FeeStructureResponse(
				s.getId(), s.getName(), s.getStatus(), s
						.getItems().stream().map(i -> new FeeStructureItemResponse(i.getId(),
								i.getFeeComponent().getName(), i.getFrequency(), i.getAmount()))
