...
public interface InvoiceRepository extends JpaRepository<Invoice, Long> {
	...
	Page<Invoice> searchIssued(@Param("status") InvoiceStatus status, @Param("yearId") Long yearId,
			@Param("term") String term, Pageable pageable);

	@Query("select distinct i from Invoice i join fetch i.studentEnrollment e join fetch e.student s join fetch e.grade g join fetch i.academicYear y left join fetch i.items where i.id=:id")
	...
	@Query("select count(i) from Invoice i where i.academicYear.id=:yearId and i.status=:status and i.studentEnrollment.id=:enrollmentId and i.billingMonth=:billingMonth")
	long countMonthlyPeriod(@Param("yearId") Long yearId, @Param("status") InvoiceStatus status,
			@Param("enrollmentId") Long enrollmentId, @Param("billingMonth") LocalDate billingMonth);

	@Query("select i from Invoice i join fetch i.items where i.id=:id")
