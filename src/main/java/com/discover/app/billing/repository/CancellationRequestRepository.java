...
public interface CancellationRequestRepository extends JpaRepository<InvoiceCancellationRequest, Long> {
	...
	@Query("select r from InvoiceCancellationRequest r join fetch r.invoice i join fetch i.studentEnrollment e join fetch e.student s where r.status=:status order by r.createdAt asc, r.id asc")
	List<InvoiceCancellationRequest> findDetailedByStatus(@Param("status") CancellationRequestStatus status);

	@Query("select r from InvoiceCancellationRequest r join fetch r.invoice i join fetch i.studentEnrollment e join fetch e.student s order by r.createdAt desc, r.id desc")
	List<InvoiceCancellationRequest> findAllDetailed();

	@Query("select r from InvoiceCancellationRequest r join fetch r.invoice i where r.id=:id")
