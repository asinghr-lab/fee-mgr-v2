...
@Entity @Table(name="invoices", uniqueConstraints=@UniqueConstraint(name="uk_invoice_number",columnNames="invoice_number"), indexes={@Index(name="idx_invoice_enrollment_period",columnList="student_enrollment_id,academic_year_id,billing_month,status"),@Index(name="idx_invoice_generation_date",columnList="generation_date")})
public class Invoice {
 @Id @GeneratedValue(strategy=GenerationType.IDENTITY) private Long id;
 @Column(name="invoice_number",nullable=false,length=40) private String invoiceNumber;
 @ManyToOne(fetch=FetchType.LAZY,optional=false) @JoinColumn(name="student_enrollment_id",nullable=false) private StudentEnrollment studentEnrollment;
