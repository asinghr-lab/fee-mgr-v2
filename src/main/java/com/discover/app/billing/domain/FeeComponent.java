...
public class FeeComponent {
    ...
    public FeeComponent(String name){this.name=name;}
    @PrePersist void onCreate(){var now=LocalDateTime.now();createdAt=now;updatedAt=now;}
    @PreUpdate void onUpdate(){updatedAt=LocalDateTime.now();}
    public Long getId(){return id;} public String getName(){return name;} public FeeStatus getStatus(){return status;} public LocalDateTime getCreatedAt(){return createdAt;} public LocalDateTime getUpdatedAt(){return updatedAt;}
    public void deactivate(){if(status==FeeStatus.INACTIVE) throw new IllegalStateException("Fee component is already inactive"); status=FeeStatus.INACTIVE;}
}
